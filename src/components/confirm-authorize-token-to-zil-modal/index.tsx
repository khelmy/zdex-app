...
const AuthorizeTokenToZilModal: React.FunctionComponent<IProps> = (props) => {
  const {
    tokenAddress,
    amount,
    gasPrice,
  ...
  useEffect(
    () => {
      ...
      setPrevApproveTokenStatus(authorizeTokenToZilStatus);
    },
    [authorizeTokenToZilStatus, prevApproveTokenStatus]
  );
  ...
  const onSubmit = (e) => {
    ...
    authorizeTokenToZil(tokenAddress, amount, gasPrice);
  };
          ...
          <div>
            ...
            {authorizeTokenToZilId ? (
              <u>
                <a
                  target="_blank"
                  href={`${EXPLORER_URL}/transactions/${authorizeTokenToZilId}`}
                  rel="noreferrer"
                >
...
export default AuthorizeTokenToZilModal;
